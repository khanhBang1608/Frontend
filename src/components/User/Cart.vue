...
<div v-else>
    ...
    <tbody>
      <tr v-for="item in cart" :key="item.id" class="cart-item-row-modern">
        <td class="text-center">
          <img
            :src="'/images/' + item.product.imageNames[0]"
            alt="Ảnh sản phẩm"
        ...
        <td class="text-center">
          <button class="cart-remove-btn-modern" @click="handleRemoveFromCart(item.productSize.id)">
            Xoá
          </button>
        </td>
      </tr>
  ...
  <div class="text-end cart-total-price-modern">
    Tổng tiền:
    {{
      cart.reduce((total, item) => total + item.product.price * item.quantity, 0).toLocaleString('vi-VN')
    }}₫
  </div>
